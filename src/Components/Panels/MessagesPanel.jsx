...
const MessagesPanel = ({ openChat, messages, getUserInfo, activeChat, unreadChats, handleNewChatClick }) => {
    ...
    if (Object.keys(messages).length > 1) {
        messagesList = Object.entries(messages).filter(([id, msgs]) => {
            ...
            return <UserButton avatarColor={user.color} avatarContent={user.nick} title={user.nick} subtext={messages[id].length > 0 ? messages[id][messages[id].length - 1].msg[messages[id][messages[id].length - 1].msg.length - 1] : undefined} unread={unreadChats.has(id)} selected={id === activeChat && appSize !== "small"} hoverable onClick={() => { openChat(id) }} key={id} />;
        });
    } else {
    ...
    return (
        <div id="messagesMenu" className={appSize === "small" ? "mobile" : undefined}>
            ...
            <UserButton avatarColor="grey" avatarContent={<FontAwesomeIcon icon={solid('users')} />} title="Global chat" subtext="Chat with all users" unread={unreadChats.has("global")} selected={activeChat === "global" && appSize !== "small"} hoverable onClick={() => { openChat('global') }} />
